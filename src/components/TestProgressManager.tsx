    ...
    <View className="absolute top-0 left-0 right-0 z-50">
      ...
      {isActive && progress && (
        <View className={`px-4 py-2 items-center ${
          themeMode === 'cyberpunk' 
            ? 'bg-black/90 border-b border-cyan-400/30' 
            : themeMode === 'dark' 
            ? 'bg-gray-900/90' 
            : 'bg-black/80'
        }`}>
          <Text className={`text-xs font-medium ${
            ...
            {isSaving 
              ...
              : hasUnsavedChanges 
              ? (themeMode === 'cyberpunk' ? 'UNSAVED CHANGES' : 'Unsaved changes')
              : (themeMode === 'cyberpunk' ? 'SAVED' : 'Saved')
            }
          </Text>
          {lastSaveTime && (
            <Text className={`text-xs mt-0.5 ${
              themeMode === 'cyberpunk' 
                ? 'text-cyan-400/70 tracking-wider' 
                : themeMode === 'dark' 
                ? 'text-white/70' 
                : 'text-white/70'
            }`}>
          ...
          {saveError && (
            <Text className={`text-xs mt-0.5 ${
              ...
              {themeMode === 'cyberpunk' ? 'SAVE ERROR' : saveError}
            </Text>
          )}
        </View>
      )}
