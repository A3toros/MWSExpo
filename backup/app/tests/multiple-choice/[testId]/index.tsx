  ...
  const submitTest = useCallback(async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const retestKey = `retest1_${user.student_id}_multiple_choice_${testId}`;
        const retestAssignKey = `retest_assignment_id_${user.student_id}_multiple_choice_${testId}`;
          ...
          <View style={styles.resultsContainer}>
            ...
            {testResults.questions.map((result: any, index: number) => (
              <View key={index} style={styles.questionResult}>
                ...
                <Text style={styles.questionText}>{result.questionText || result.question}</Text>
